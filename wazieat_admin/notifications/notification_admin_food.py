...
"""
def send_sms_code(request, user, type1):
    # send SMS

    if type1 == 'food':
        message = "d'un nouveau plat"
    else:
        message = "d'une nouvelle boisson"
    ...
    body = {
        "outboundSMSMessageRequest": {
            ...
            "outboundSMSTextMessage": {
                "message": "Hello! C'est WaziEats, \nVous avez une demande " + message + " à traiter.\n\nCordialement"
            }
        }
    }
    headers = {
...
def send_mail_code(request, user, type1, name, email, restaurant):

    url = settings.APP['admin_url'] + 'login'

    if type1 == 'food':
        subject = 'Nouveau plat du restaurant ' + name
        type1 = "du plat"
    else:
        subject = 'Nouvelle boisson du restaurant ' + name
        type1 = "de la boisson"
    ...
    html_content = render_to_string(
        'email/accounts/notif_admin_food.html',
        context
    )
    text_content = render_to_string(
        'email/accounts/notif_admin_food.txt',
        context
    )
...
def notifications_admin_food(request, user, type1, name, email, restaurant):
    send_sms_code(request, user, type1)
    if user.email is not None:
        send_mail_code(request, user, type1, name, email, restaurant)
