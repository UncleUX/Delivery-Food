...
def send_sms_message(user, password):
    ...
    message = message + "Utilisez ce mot de passe pour vous connecter: " + str(password) + "\n"
    body = {
...
def send_mail_message(request, user, password):
    context = {
        'phone': user.phone,
    ...
    html_content = render_to_string(
        'email/accounts/send_delivery_password.html',
        context
    )
...
def notifications(request, user, password):
    send_sms_message(user, password)
    if user.email is not None:
        send_mail_message(request, user, password)
