...
def send_mail_message(request, user):
    context = {
        'phone': user.phone,
        'site_name': settings.APP['site_name'],
    }

    subject = "Confirmation de l'initialisation du compte"

    html_content = render_to_string(
        'email/accounts/confirm_initAccount_delivery.html',
        context
    )
    """
    text_content = render_to_string(
        'email/accounts/confirm_initAccount_delivery.txt',
        context
    )
    """
...
def notifications(request, user, message_sms):
    send_sms_message(user, message_sms)
    if user.email is not None:
        send_mail_message(request, user)
